// Generate data/sets/twm.js from twm-cards.json (LimitlessTCG extract)
const fs = require('fs');

const list = JSON.parse(fs.readFileSync('C:/Users/User/Downloads/twm-cards.json', 'utf8'))
  .sort((a, b) => a.num - b.num);
if (list.length !== 226) throw new Error('expected 226, got ' + list.length);

const vFor = c => {
  if (c.num <= 167) {
    if (c.rarity === 'Double Rare') return 'ex';
    if (c.rarity === 'Ultra Rare') return 'ace';    // in-set ACE SPECs (pink)
  ...
  return 'secret';                                   // Secret Rare golds
};
...
const slug = name => name.toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9\s-]/g, '').trim().replace(/\s+/g, '-').replace(/-+/g, '-');
...
const rh = slots.filter(s => s.v === 'rh').length;
const stats = {};
...
registerSet('twm', {
  binder: {
    ...
    subtitle: '226 cards · 167 main + 59 secret · Master Set incl. ${rh} reverse holos',
...
console.log('written data/sets/twm.js | total for catalog:', slots.length);
